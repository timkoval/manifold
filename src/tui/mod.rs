...
impl TuiApp {
    ...
    fn render_overview(&self, f: &mut Frame, area: Rect, spec_row: &SpecRow) {
        let spec: SpecData = match serde_json::from_value(spec_row.data.clone()) {
            Ok(s) => s,
            Err(_) => {
                let p = Paragraph::new("Invalid spec data (cannot render overview)")
    ...
    fn render_requirements(&self, f: &mut Frame, area: Rect, spec_row: &SpecRow) {
        let spec: SpecData = match serde_json::from_value(spec_row.data.clone()) {
            Ok(s) => s,
            Err(_) => {
                let p = Paragraph::new("Invalid spec data (cannot render)")
                    .block(Block::default().borders(Borders::ALL).title("Invalid Spec"));
                f.render_widget(p, area);
                return;
            }
        };
        ...
        if let Some(conflict_idx) = self.conflict_list_state.selected() {
            if let Some(conflict) = self.conflicts.get(conflict_idx) {
                ...
                let manual_value: serde_json::Value = if self.manual_edit_input.trim().is_empty() {
                    ...
                    match serde_json::from_str(&self.manual_edit_input) {
                        Ok(v) => v,
                        Err(_) => serde_json::Value::String(self.manual_edit_input.clone()),
                    }
                };
